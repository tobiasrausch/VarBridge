...
using namespace varbridge;


inline void
displayUsage() {
  std::cerr << "Usage: varbridge <command> <arguments>" << std::endl;
  std::cerr << std::endl;
  std::cerr << "    lift        lift variants across assemblies" << std::endl;
  std::cerr << std::endl;
}

int main(int argc, char **argv) {
    if (argc < 2) { 
      printTitle("VarBridge");
      displayUsage();
      return 0;
    }

    if ((std::string(argv[1]) == "version") || (std::string(argv[1]) == "--version") || (std::string(argv[1]) == "--version-only") || (std::string(argv[1]) == "-v")) {
      std::cerr << "VarBridge version: v" << varbridgeVersionNumber << std::endl;
    ...
    else if ((std::string(argv[1]) == "lift")) {
      return lift(argc-1,argv+1);
    }
